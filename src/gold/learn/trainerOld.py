...
class FeatureExtractor():
    ...
    def extract_features(self, start, move, movePosition, isblack, outcome):
        ...
        patchEx.setPatchSize(4)
        patches = patchEx.calculate_feature()

        for row in patches:
          if isblack:
            fout=open('extraneous/games/patchesTraining4BtL.csv', 'a')
            fout.write(','.join([str(x) for x in row]+outcome))
            fout.write('\n')
            fout.close()
          else:
            fout=open('extraneous/games/patchesTraining4WtK.csv', 'a')
            fout.write(','.join([str(x) for x in row]+outcome))
            fout.write('\n')
            fout.close()

        patchEx.setPatchSize(5)
        patches = patchEx.calculate_feature()

        for row in patches:
          if isblack:
            fout=open('extraneous/games/patchesTraining5BtL.csv', 'a')
            fout.write(','.join([str(x) for x in row]+outcome))
            fout.write('\n')
            fout.close()
          else:
            fout=open('extraneous/games/patchesTraining5WtK.csv', 'a')
            fout.write(','.join([str(x) for x in row]+outcome))
            fout.write('\n')
            fout.close()
